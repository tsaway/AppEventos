...
//imports files
import {
    SendSales, GetListReleaseSales, DeleteItemReleaseSales
                ...
                <View style={styles.container}>
                    ...
                    <View style={styles.control}>
                        <TextInput keyboardType='numeric' placeholderTextColor='#9b9b9b'
                            placeholder='Dinheiro: ' value={this.state.moneyValue} style={styles.input}
                            onChangeText={(txt) => this.setState({ moneyValue: txt })} />
                        <TextInput keyboardType='numeric' placeholderTextColor='#9b9b9b'
                            placeholder='Cartão débito: ' value={this.state.cardDebitValue} style={styles.input}
                            onChangeText={(txt) => this.setState({ cardDebitValue: txt })} />
                        <TextInput keyboardType='numeric' placeholderTextColor='#9b9b9b'
                            placeholder='Cartão crédito: ' value={this.state.cardCreditValue} style={styles.input}
                            onChangeText={(txt) => this.setState({ cardCreditValue: txt })} />
                        <TouchableHighlight underlayColor='#9b9b9b' style={styles.btnDate}
                            onPress={() => this.setState({ isDateVisible: true })}>
                            <Text style={{ fontSize: 16 }}>{this.state.selectedDate}</Text>
                        </TouchableHighlight>
                        <DateTimePicker
                        ...
                        />
                        <TouchableHighlight onPress={() => this.props.SendSales(
                        ...
                        <TouchableHighlight
                            onPress={() => this.props.GetListReleaseSales(list =>
                                this.setState({ list, isModalVisible: true }))}
                            underlayColor='#5FC7EA' style={styles.btn}>
                            <Text style={styles.txtBtn}>Verificar lista de vendas</Text>
                        </TouchableHighlight>
                    </View>
                    ...
                    <Modal animationType='slide' visible={this.state.isModalVisible}>
                        ...
                        <View style={styles.modal}>
                            ...
                            <FlatList
                                ...
                                renderItem={({ item }) =>
                                    <SalesReleasesItem data={item}
                                        deleteItem={this.props.DeleteItemReleaseSales} />}
                            />
                        </View>
                    </Modal>
                </View>
...
const SalesConnect = connect(mapStateToProps, {
    SendSales, GetListReleaseSales, DeleteItemReleaseSales
})(Sales)
