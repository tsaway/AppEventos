...
import {
    Text, View, StyleSheet, Image, TextInput, TouchableHighlight, ScrollView, Modal, Picker
} from 'react-native'
...
export class EventRegistration extends Component {
    constructor(props) {
        ...
        this.props.GetDataDefault((listName, listCity, listEstado, listPromoter) => this.setState({
            listName, listCity, listEstado, listPromoter
        }))
    }

    render() {
        let nameItems = this.state.listName.map((v, k) => {
            return <Picker.Item key={k} value={k} label={v.name} />
        })

        let cityItems = this.state.listCity.map((v, k) => {
            return <Picker.Item key={k} value={k} label={v.city} />
        })

        let estadoItems = this.state.listEstado.map((v, k) => {
            return <Picker.Item key={k} value={k} label={v.estado} />
        })

        let promoterItems = this.state.listPromoter.map((v, k) => {
        ...
        return (
            <ScrollView style={styles.scrollMain} >
                <View style={styles.container}>
                    <TouchableHighlight underlayColor='transparent' style={styles.btnDataDefault}
                        onPress={() => this.setState({ isDataDefaultVisible: true })}>
                        <Image
                            source={require('./../../assets/images/icons/arrow/navigation.png')}
                            style={styles.imgDataDefault}
                        />
                    </TouchableHighlight>
                    <TextInput placeholderTextColor='#9b9b9b' value={this.state.name}
                        style={styles.input} placeholder='Nome:'
                        onChangeText={(txt) => this.setState({ name: txt })} />
                    <TextInput placeholderTextColor='#9b9b9b' value={this.state.city}
                        style={styles.input} placeholder='Cidade:'
                        onChangeText={(txt) => this.setState({ city: txt })} />
                    <TextInput placeholderTextColor='#9b9b9b' value={this.state.estado}
                        style={styles.input} placeholder='Estado:'
                    ...
                    <DateTimePicker
                        ...
                        onCancel={() => this.setState({ isDateFinalVisible: false })}
                    />
                    <TouchableHighlight underlayColor='#5FC7EA' style={styles.btn}
                        onPress={() => {
                            ...
                            this.props.RegisterEvent(
                                ...
                                () => { this.setState({ isLoadRegisterEvent: false }) })
                        }}>
                        <Text style={styles.txtBtn}>Cadastrar evento</Text>
                    </TouchableHighlight>
                    {/* --------------- Modal Lista dataDefault --------------- */}
                    <Modal animationType='slide' visible={this.state.isDataDefaultVisible}>
                        <ScrollView style={{ flex: 1 }}>
                            <View style={styles.exit}>
                                <TouchableHighlight
                                    onPress={() => this.setState({ isDataDefaultVisible: false })}
                            ...
                            <View style={styles.modal}>
                                ...
                                <View>
                                    ...
                                    <Picker
                                        selectedValue={this.state.pickerValueEstado}
                                        style={{ height: 50, width: 250 }}
                                        onValueChange={itemValue =>
                                            this.setState({ pickerValueEstado: itemValue })}>
                                        {estadoItems}
                                    </Picker>
                                </View>
                                <View>
                                    <Text style={styles.modalTitle}>Promotora</Text>
                                    <Picker
                                        selectedValue={this.state.pickerValuePromoter}
                                        style={{ height: 50, width: 250 }}
                                        onValueChange={itemValue =>
                                            this.setState({ pickerValuePromoter: itemValue })}>
                                        {promoterItems}
                                    </Picker>
                                </View>
                                <TouchableHighlight onPress={() => {
                                    ...
                                    if (this.state.pickerValueName === 0) {
                                        ...
                                        let newName = this.state.listName[this.state.pickerValueName].name
                                    ...
                                    if (this.state.pickerValueCity === 0) {
                                        ...
                                        let newCity = this.state.listCity[this.state.pickerValueCity].city
                                        this.state.city = newCity
                                        this.setState(this.state)
                                    }

                                    if (this.state.pickerValueEstado === 0) {
                                        this.state.validation[2].estado = 0
                                        this.setState(this.state)
                                    } else {
                                        this.state.validation[2].estado = 1
                                        let newEstado = this.state.listEstado[this.state.pickerValueEstado].estado
                                        this.state.estado = newEstado
                                        this.setState(this.state)
                                    }

                                    if (this.state.pickerValuePromoter === 0) {
                                        this.state.validation[3].promoter = 0
                                        this.setState(this.state)
                                    } else {
                                        this.state.validation[3].promoter = 1
                                        let newPromoter = this.state.listPromoter[this.state.pickerValuePromoter].promoter
                                        this.state.promoter = newPromoter
                                        this.setState(this.state)
                                    }
                                    this.props.SendDataDefault(() =>
                                        this.setState({ isDataDefaultVisible: false }))
                                }} underlayColor='#5FC7EA' style={styles.btnModal}>
...
const EventRegistrationConnect = connect(mapStateToProps, {
    RegisterEvent, GetDataDefault, SendDataDefault
})(EventRegistration)
