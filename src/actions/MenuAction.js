                        ...
                        text: 'Sim, quero finalizar', onPress: () => {
                            dir.child('eventActive').set({ eventActive: 0 }).then(() => {
                                ...
                                dir.child('eventCurrent').once('value').then(snapshot => {
                                    ...
                                    let releases = snapshot.val().releases //add line
                                    callback(name, city, estado, promoter, dateInitial,
                                        dateFinal, releases)
                                })
                            }).catch(e =>
...
export const SaveEvent = (name, city, estado, promoter, dateInitial, dateFinal, releases) => {
    return () => {
...
export const GetExpensesRegisted = callback => {
    return dispatch => {
        ...
        let listExpensesRegisted = []

        dir.child('expensesRegisted').once('value').then(snapshot => {
